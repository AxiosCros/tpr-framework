...
class Route
{
     ...
     * 设置或者获取路由标识.
     *
     * @param array|string $name  路由命名标识 数组表示批量设置
     ...
     * 注册路由分组.
     *
     * @param array|string   $name    分组名称或者参数
     ...
     */
    public static function check(Request $request, $url, $depr = '/', $checkDomain = false)
    {
...
//            $vars    = $item['var'];
            ...
            if (\is_array($rule)) {
                ...
                self::setOption($option);
                if (isset($options['bind_model'], $option['bind_model'])) {
     ...
     * @return bool
     */
    private static function checkOption($option, Request $request)
    {
